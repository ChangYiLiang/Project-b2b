						...
						<div class="portlet-body">
							<div class="table-container">
								<table class="table table-striped table-bordered table-hover" id="datatable_cl">
									...
									<#list goodstag as gtt>
									<tr>
										<td>${gtt.goodsTag}</td>
										<td>${gtt.goodsAttr}</td>
										<td>
											<a class="btn btn-primary pull-right" style="margin-left:20px" href="goodstagupdate.do?id=${gtt.id}">查看/修改</a>
											<#if gtt.isShow=1>									            					
								            	<label class="btn btn-primary pull-right label-danger" name="bind" data="${gtt.id}">点击隐藏</label>
								            <#else>
								            	<label class="btn btn-primary pull-right" name="bind2" data="${gtt.id}">点击显示</label>
								            </#if>
										</td>
									</tr>
									</#list>
	...
	$("label[name=bind]").click(function(){
		...
		if (confirm(msg)==true){
			...
			$.ajax({url:url,data,success:function(data){
				if(data == "success"){
					alert("解除成功");
					window.location.href="goodstaglist.do?id=${goods.id}";
				} else {
					alert("解除失败");
					return ;			
				}
