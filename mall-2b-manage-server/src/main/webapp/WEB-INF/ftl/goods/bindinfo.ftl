					            			...
					            			<ul class="list-group">
						            			<#list mapp[val] as v>
						            				<li class="list-group-item" style="height:50px;line-height:33px">									            									
						            					...
						            					<#if v.isShow=1>									            					
								            				<label class="btn btn-primary pull-right label-danger jj" name="bind" data="${v.id}">解除绑定</label>
								            			<#else>
								            				<label class="btn btn-primary pull-right jj" name="bind2" data="${v.id}">重新绑定</label>
								            			</#if>
						            					<button class="btn btn-primary pull-right gg" style="margin-left:40px;position:relative;left:-30px">排序</button>	
						            					<input type="num" value="${v.sort}" style="width:40px;display:inline-block;" class="bibi form-control pull-right">							            									            											    								            				
						            				</li>
						            			</#list>
					            			</ul>
				            			</li>
...
$(function(){
	...
	$(".gg").click(function(){
		var msg = "确定要排序该配件吗？\n\n请确认！";
		if (confirm(msg)==true){
			var url  ="toupdatesort.do";
			var data={
					"goodsId":${goods.id},
					"itemId":$(this).prev(".jj").attr("data"),
					"sort":$(this).next(".bibi").val()
				};
			$.ajax({url:url,data,success:function(data){
				if(data == "success"){
					alert("排序成功");
